...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const deepLinkUrl = `umbraa://auth?token=${encodeURIComponent(transferToken)}&origin=${encodeURIComponent(origin)}&desktop_state=${encodeURIComponent(stateMetadata.desktopAuthState)}`;
...
<head>
  <meta charset="utf-8">
  <title>Redirecting to Umbraa...</title>
  <style>
    body {
...
<body>
  <div class="container">
    ...
    <p>${safeMessage}</p>
    <a href="umbraa://auth?error=auth_failed">Return to App</a>
  </div>
</body>
</html>`;
}
